...
folderName <- "data/newDataCollection/"
newData <- data.table()
...
for(i in 1:length(files)){
  dataX <- fread(paste0("data/newDataCollection/",files[i]))
...
# fwrite(dataX,"data/processedData/allData.csv")

# dataX[,..fieldNames]
####merge two readings and remove duplicates
oldData <- fread("data/processedData/allData.csv")
...
fwrite(allData,"data/processedData/allData.csv")
fwrite(resumeTab,"data/processedData/qualCheck.csv")
# 
# 
# 
# ##compute daily mean
...
fwrite(dailyData, file = "data/processedData/dailyData.csv")


####Move processed csv file to storedData folder and remove original folder
files <- list.files(path= folderName)
file.copy(paste0("data/newDataCollection/",files), "data/storedData/", recursive=TRUE)
unlink(paste0("data/newDataCollection/",files), recursive=TRUE)
